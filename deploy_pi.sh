#!/usr/bin/env bash
# Rsync the drone-side code to the Raspberry Pi and (optionally) launch a drone.
#
#   ./deploy_pi.sh                        # sync only
#   ./deploy_pi.sh drone-01               # sync + start drone-01
#   MQTT_BROKER=192.168.1.42 ./deploy_pi.sh drone-01
#
# Requires passwordless SSH to ${PI_USER}@${PI_HOST}. First time, copy your
# pubkey with `ssh-copy-id group8@group8-drone.local`.
set -euo pipefail
...
if [ $# -ge 1 ]; then
  ...
  echo "  ssh ${PI_USER}@${PI_HOST} 'tail -f ${REMOTE_DIR}/${drone_id}.log'"
fi
