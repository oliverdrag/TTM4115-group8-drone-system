"""SQLite persistence for drones, orders and missions.

This is the Mission & Drone Inventory DB from the deployment diagram. The
fleet manager keeps live state in memory but mirrors the interesting bits
here so we have a record across restarts.
"""
...
class Database:
    ...
    def list_drones(self) -> list[dict]:
        with self._lock:
            rows = self._conn.execute("SELECT * FROM drones").fetchall()
    ...
    def update_order(self, order_id: int, **fields) -> None:
        ...
        with self._lock:
            self._conn.execute(f"UPDATE orders SET {cols} WHERE id=?", values)
            self._conn.commit()
    ...
    def update_mission(self, mission_id: int, **fields) -> None:
        ...
        with self._lock:
            self._conn.execute(f"UPDATE missions SET {cols} WHERE id=?", values)
            self._conn.commit()
        ...
        with self._lock:
            row = self._conn.execute(
                ...
                SELECT * FROM missions
                 WHERE drone_id=? AND status NOT IN ('completed','cancelled','failed')
