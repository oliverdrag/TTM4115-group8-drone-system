"""Drone-side inngangspunkt. Kjører på  Raspberry Pien.

Kobler sammen tre tilstandsmaskiner  (flight control + batteri) og  en
mocka sensor tjenste (navigasjons modul), alle drevet av MQTT meldinger
fra applikasjons servern.

Den fysiske Pi-drona abonnerer også på alle andre dronenes telemetri
sånn at Sense HAT kan bytte fokus via joysticken — venstre/høyre for å
bytte dron, opp/ned for zoom, midten for full-oversikt.

Bruk:
    python -m drone.drone_main drone-01
    # eller
    DRONE_ID=drone-01 python -m drone.drone_main
"""
...
class Drone:
    """Eier MQTT klienten og sender komandoer til riktige  maskina."""
    ...
    def __init__(self, drone_id: str, home: tuple[int, int]):
        ...
        # Default the Sense HAT's viewport to this drone on startup — so the
        # very first thing the LED matrix shows is just the local drone,
        # not an empty map or a stale fleet-wide view.
        ...
        lwt_topic = drone_topic(drone_id, "status")
        ...
        flight_handlers, flight_machine = build_flight(
            drone_id=drone_id,
        ...
        battery_handlers, battery_machine = build_battery(
            drone_id=drone_id,
            tick_ms=BATTERY_TICK_MS,
            publish=self._publish_battery,
        )
        self.flight = flight_handlers
        self.battery = battery_handlers
    ...
    def _publish_status(self, status: str) -> None:
        ...
        # Let the display auto-clear the path when we dock.
    ...
    def _handle_battery_empty(self) -> None:
        """Drone ran out mid-flight — abort, don't keep magic-flying on empty."""
        ...
        if flight_state not in _IN_FLIGHT_FLIGHT_STATES:
            return
        log.warning(
            "[%s] battery EMPTY while in flight state %s — emergency landing",
    ...
    def _on_focus_change(self, drone_id: str) -> None:
        log.info("[%s] viewport now following %s", self.drone_id, drone_id)
        self._publish_viewer_state(drone_id)
        # Best-effort fetch of the focused drone's current route so the
        # Sense-HAT can show the planned path lines. Runs off-thread so the
        # joystick callback doesn't block on HTTP.
        threading.Thread(
            target=self._fetch_and_set_focus_path,
            args=(drone_id,),
            daemon=True,
        ).start()
    ...
    def _fetch_and_set_focus_path(self, drone_id: str) -> None:
        try:
            r = requests.get(
                f"{APP_SERVER_URL}/api/missions/{drone_id}/path",
                timeout=3,
            )
            if not r.ok:
                return
            ...
            log.warning("[%s] could not fetch path for %s: %s",
                        self.drone_id, drone_id, e)
            return
        if route:
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties=None):
        ...
        # Fleet-wide telemetry/status so the Sense HAT can render every drone
        # and the joystick can switch focus between them.
        ...
        # Seed the display with our own home position + advertise the
        # viewer state so the hospital UI shows the correct watching marker.
    ...
    def _on_message(self, client, userdata, msg):
        try:
            ...
            log.warning("[%s] dropped invalid payload on %s", self.drone_id, msg.topic)
            return

        parts = msg.topic.split("/")
        # Shape: ttm4115/group8/drone/<id>/<channel>
        if len(parts) < 5:
            return
        src_drone = parts[3]
        channel = parts[4]

        # Someone else's telemetry/status — feed the display, don't touch
        # our state machines.
    ...
    def _fetch_grid_and_paint(self) -> None:
        try:
            ...
            log.warning("[%s] could not fetch grid from %s: %s", self.drone_id, APP_SERVER_URL, e)
            return
        width = int(data.get("width", 0))
        height = int(data.get("height", 0))
        zones = data.get("zones", [])
        self.display.set_grid(width, height, zones)
        self.display.set_position(self.home[0], self.home[1], drone_id=self.drone_id)
    ...
    def run(self, broker: str = MQTT_BROKER, port: int = MQTT_PORT) -> None:
        ...
        # Joystick on the Sense HAT — a no-op when the hardware isn't there.
...
def main(argv: list[str]) -> None:
    ...
    home = tuple(cfg["home"])
    drone = Drone(drone_id=cfg["id"], home=home)
    drone.run()
